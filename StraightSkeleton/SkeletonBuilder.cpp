...
template<class _Set, class _Kty>
static bool SetContains(const _Set& s, const _Kty& k)
{
	return s.find(k) != s.end();
}

template<class _Set, class _Kty>
static bool SpSetContains(std::shared_ptr<_Set> s, const _Kty& k)
{
	return s->find(k) != s->end();
}
...
Skeleton SkeletonBuilder::Build(listVector2d& polygon, nestedlistVector2d& holes)
{
	...
	while (!queue->empty())
	{
		...
		for (auto event : *eventQueue)
		{
			...
			if (event->GetType() == SE_Edge)
				throw std::runtime_error("All edge@events should be converted to MultiEdgeEvents for given level");
			if (event->GetType() == SE_Split)
				throw std::runtime_error("All split events should be converted to MultiSplitEvents for given level");
			if (event->GetType() == SE_MultiSplit)
				EmitMultiSplitEvent(std::static_pointer_cast<MultiSplitEvent>(event), sLav, queue, edges);
			else if (event->GetType() == SE_Pick)
				EmitPickEvent(std::static_pointer_cast<PickEvent>(event));
			else if (event->GetType() == SE_MultiEdge)
				EmitMultiEdgeEvent(std::static_pointer_cast<MultiEdgeEvent>(event), queue, edges);
			else
...
{
	if (chainBegin->GetType() == ICT_SingleEdge)
	{
	...
	if (chainEnd->GetType() == ICT_SingleEdge)
	{
		// left face
		if (lastFaceNode == nullptr)
		{
...
void SkeletonBuilder::CreateOppositeEdgeChains(sp<hashsetCircularList> sLav, sp<listIChain> chains, sp<Vector2d> center)
{
	...
	for (auto chain : *chains)
	{
		...
		if (chain->GetType() == ICT_Split)
		{
			auto splitChain = std::static_pointer_cast<SplitChain>(chain);
			auto oppositeEdge = splitChain->OppositeEdge();
			if (oppositeEdge != nullptr && !SetContains(*oppositeEdges, oppositeEdge))
			{
...
std::shared_ptr<std::vector<std::shared_ptr<IChain>>> SkeletonBuilder::CreateChains(sp<listSkeletonEvent> cluster)
{
	...
	{
		if (skeletonEvent->GetType() == SE_Edge)
		{
			auto e = std::static_pointer_cast<EdgeEvent>(skeletonEvent);
			edgeCluster->push_back(e);
		}
		else
		{
			if (skeletonEvent->GetType() == SE_VertexSplit)
			{
				// It will be processed in next loop to find unique split
				// events for one parent.
			}
			else if (skeletonEvent->GetType() == SE_Split)
			{
				auto splitEvent = std::static_pointer_cast<SplitEvent>(skeletonEvent);
	...
	for (auto skeletonEvent : *cluster)
	{
		if (skeletonEvent->GetType() == SE_VertexSplit)
		{
			auto vertexEvent = std::static_pointer_cast<VertexSplitEvent>(skeletonEvent);
			if (!SetContains(*vertexEventsParents, vertexEvent->Parent))
			{
...
bool SkeletonBuilder::IsEventInGroup(sp<hashsetVertex> parentGroup, sp<SkeletonEvent> event)
{
	if (event->GetType() == SE_Split)
	{
		auto splitEvent = std::static_pointer_cast<SplitEvent>(event);
		return SpSetContains(parentGroup, splitEvent->Parent);
	}
	else if (event->GetType() == SE_Edge)
	{
		auto edgeEvent = std::static_pointer_cast<EdgeEvent>(event);
		return SpSetContains(parentGroup, edgeEvent->PreviousVertex) || SpSetContains(parentGroup, edgeEvent->NextVertex);
	}
	return false;
}
...
std::shared_ptr<SkeletonEvent> SkeletonBuilder::CreateLevelEvent(sp<Vector2d> eventCenter, double distance, sp<listSkeletonEvent> eventCluster)
{
	...
	auto checkAndCastAsEdgeChain = [](std::shared_ptr<IChain>& chain)->std::shared_ptr<EdgeChain> {
		if (chain->GetType() == ICT_Edge)
			return std::static_pointer_cast<EdgeChain>(chain);
		return nullptr;
	};
	...
	if (chains->size() == 1)
	{
		...
		if (chain->ChainType() == EChainType::CLOSED_EDGE)
			return std::make_shared<PickEvent>(eventCenter, distance, checkAndCastAsEdgeChain(chain));
		if (chain->ChainType() == EChainType::EDGE)
			return std::make_shared<MultiEdgeEvent>(eventCenter, distance, checkAndCastAsEdgeChain(chain));
	...
	for (auto face : *faces)
	{
		if (face->Size() > 0)
		{
			...
			for (auto fn : *face)
			{
				...
				if (!SetContains(*distances, *point))
